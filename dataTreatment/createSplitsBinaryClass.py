...
def create_balanced_folders(directories, sizes):
    ...
    for size in sizes:
        ...
        benign_output_file = os.path.join(size_folder, f"benign_{size}_lines.csv")
        benign_split_df.to_csv(benign_output_file, index=False)
        print(f"Saved {benign_output_file} with {len(benign_split_df)} lines")

        malicious_output_file = os.path.join(size_folder, f"malign_{size}_lines.csv")
        malicious_split_df.to_csv(malicious_output_file, index=False)
        print(f"Saved {malicious_output_file} with {len(malicious_split_df)} lines")
